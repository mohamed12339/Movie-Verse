      ...
      child: BlocBuilder<MoviesCubit, MoviesState>(
        builder: (context, state) {
          if (state.genresApiState.hasData &&
              state.genresApiState.myData.isNotEmpty) {
            if (_selectedGenre == null) {
              _selectedGenre = state.genresApiState.myData.first;
              BlocProvider.of<MoviesCubit>(
                context,
              ).getMoviesByGenre(_selectedGenre!);
            }
            return buildContent(genres: state.genresApiState.myData.toList());
            ...
            if (state.moviesByGenreApiState is SuccessApiResult &&
              ...
              return Expanded(
                child: MoviesGridView(
                  movies: state.moviesByGenreApiState.myData,
                ),
              );
            } else if (state.moviesByGenreApiState.hasError) {
              return Expanded(
                child: ErrorView(
                  message: state.moviesByGenreApiState.myError.message,
                ),
              );
            } else {
